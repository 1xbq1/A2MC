...
class Feeder(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #data_numpy_v3 = augmentations.temporal_cropresize(data_numpy,number_of_frames, self.l_ratio, self.input_size)
        ...
        if self.input_representation == "motion":
            ...
            #motion_v3 = np.zeros_like(data_numpy_v3)
            #motion_v3[:, :-1, :, :] = data_numpy_v3[:, 1:, :, :] - data_numpy_v3[:, :-1, :, :]

            data_numpy_v1 = motion_v1
            data_numpy_v2 = motion_v2
            #data_numpy_v3 = motion_v3
            ...
            '''bone_v3 = np.zeros_like(data_numpy_v3)
            for v1, v2 in self.Bone:
                bone_v3[:, :, v1 - 1, :] = data_numpy_v3[:, :, v1 - 1, :] - data_numpy_v3[:, :, v2 - 1, :]'''

            data_numpy_v1 = bone_v1
            data_numpy_v2 = bone_v2
            #data_numpy_v3 = bone_v3
        ...
        '''if random.random() < 0.5:
            data_numpy_v3 = augmentations.Rotate(data_numpy_v3)
        if random.random() < 0.5:
            data_numpy_v3 = augmentations.Flip(data_numpy_v3)
        if random.random() < 0.5:
            data_numpy_v3 = augmentations.Shear(data_numpy_v3)
        if random.random() < 0.5:
            data_numpy_v3 = augmentations.spatial_masking(data_numpy_v3)
        if random.random() < 0.5:
            data_numpy_v3 = augmentations.temporal_masking(data_numpy_v3)'''
